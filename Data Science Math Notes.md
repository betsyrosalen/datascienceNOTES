...
![Laws of Exponents](http://dailprice.weebly.com/uploads/1/3/2/7/13279202/2545731_orig.png)
...
***e* = the limit as n -> ∞ of (1 + 1/n)^n**

### Continuous Growth Formula
Continuous Growth can be calculated using the formula **f(x) = a*e*^rx**
where:
...
- **b** determines **the rate at which the graph grows**
    ...
    - the function will ***decrease if 0 < b < 1*** (negative rate)
...
- If **0 < b < 1** (negative rate), as x→∞, f(x) → 0 and as x → −∞, f(x) → ∞
...
### Limits
The **limit** of a function describes the behavior of the function when the variable is near, but does not equal, a specified number

We say **"the limit of f(x), as x approaches c, is L"**

lim x→c f(x) = L

lim x→c f(x) is a single number that describes the behavior of f(x) *NEAR, BUT NOT AT*, the point x = c.

If the graph at that point (n, y) is not near a single number we say, lim x→n f(x) does not exist

Just because f(x) at x = c is not defined it does not mean that lim x→c does not exist.  You can still find the limit using algebra or a graph (page 76 in textbook)

The values of a function near a point may depend on the direction we use to approach that point.  In this case there are Left and Right Limits denoted with x → c- and x → c+

### Continuity
A function that is “friendly” and doesn’t have any breaks or jumps in it is called continuous. More formally:

#### Definition of Continuity at a Point
A function f is continuous at x = a if and only if lim x→a f(x) = f(a).
...
![Discontinuity](http://tutorial.math.lamar.edu/Classes/CalcI/Continuity_files/image001.gif)
...
This is helpful, because the definition of continuity says that for a continuous function, lim x→a f(x) = f(a). That means for a continuous function, we can find the limit by direct substitution (evaluating the function) if the function is continuous at a.